...
public class NoteController : Controller
{
    ...
    {
        var orchestralSets = await _orchestralSetRepository.GetAll();
    ...
    {
        var orchestralSet = await _orchestralSetRepository.GetTById(id);
    ...
    public async Task<IActionResult> CreateOrchestralSet(OrchestralSet orchestralSet)
    {
        ...
        orchestralSet.Instruments = new List<Instrument>();

        if (orchestralSet.InstrumentsId != null)
        {
            foreach (var id in orchestralSet.InstrumentsId)
            {
                var instrument = await _instrumentRepository.GetTById(id);
        ...
        {
            orchestralSet.Links = new List<Link>();
            foreach (var url in orchestralSet.LinkTransfer) orchestralSet.Links.Add(new Link(url));
        }

        var newOrchestralSet = await _orchestralSetRepository.Create(orchestralSet);
    ...
    {
        var newInstrument = await _instrumentRepository.Create(instrument);
    ...
    {
        var deleted = await _instrumentRepository.Delete(id);
        if (deleted == false)
        {
    ...
    {
        var items = await _contributorsRepository.GetAll();
        if (items == null || !items.Any())
        {
    ...
    {
        var newItem = await _contributorsRepository.Create(item);
        if (newItem == null)
        {
            return StatusCode(500, "Internal server error while creating instrument please try again");
        }
    ...
    {
        var deleted = await _contributorsRepository.Delete(id);
        if (deleted == false)
        {
